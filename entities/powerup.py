...
class PowerUp:
    def __init__(self, x, y, powerup_type=None):
        self.x = x
        self.y = y
        self.size = int(constants.POWERUP_SIZE * constants.SCALE_FACTOR)
        self.active = True
        
        # Choose random type if not specified
        if powerup_type is None:
            self.type = random.choice(constants.POWERUP_ALL_TYPES)
        else:
        ...
        self.warning_timer = constants.POWERUP_WARNING_TIME
        ...
        if self.type == constants.POWERUP_PADDLE_SIZE:
            self.variant = random.choice(["increase_self", "decrease_enemies"])
        elif self.type == constants.POWERUP_BALL_SPEED:
            self.variant = random.choice(["slow", "fast"])
        elif self.type == constants.POWERUP_SHIELD:
            self.variant = "shield"
        elif self.type == constants.POWERUP_PADDLE_SWAP:
            self.variant = "swap"
        elif self.type == constants.POWERUP_GHOST_BALL:
            self.variant = "ghost"
        elif self.type == constants.POWERUP_MAGNETIZE:
            self.variant = "magnetize"
        elif self.type == constants.POWERUP_DECOY_BALL:
            self.variant = "decoy"
        elif self.type == constants.POWERUP_WILD_BOUNCE:
            self.variant = "wild"
        elif self.type == constants.POWERUP_CONTROL_SCRAMBLE:
            self.variant = "scramble"
        else:
            self.variant = "default"
        
    def recreate_with_scale(self):
        """Update powerup dimensions with current scale factor"""
        # Update size based on current scale factor
        self.size = int(constants.POWERUP_SIZE * constants.SCALE_FACTOR)
    ...
    def update(self):
        ...
        if self.warning_phase:
            ...
            margin = 100
            if self.x < margin or self.x > constants.SCREEN_WIDTH - margin:
                self.movement_angle = math.pi - self.movement_angle  # Reflect horizontally
                self.x = old_x
            if self.y < margin or self.y > constants.SCREEN_HEIGHT - margin:
    ...
    def get_duration(self):
        ...
        if self.type == constants.POWERUP_PADDLE_SIZE:
            return constants.POWERUP_DURATION_PADDLE_SIZE
        elif self.type == constants.POWERUP_BALL_SPEED:
            return constants.POWERUP_DURATION_BALL_SPEED
        elif self.type == constants.POWERUP_SHIELD:
            return constants.POWERUP_DURATION_SHIELD
        elif self.type == constants.POWERUP_PADDLE_SWAP:
            return constants.POWERUP_DURATION_PADDLE_SWAP
        elif self.type == constants.POWERUP_GHOST_BALL:
            return constants.POWERUP_DURATION_GHOST_BALL
        elif self.type == constants.POWERUP_MAGNETIZE:
            return constants.POWERUP_DURATION_MAGNETIZE
        elif self.type == constants.POWERUP_DECOY_BALL:
            return constants.POWERUP_DURATION_DECOY_BALL
        elif self.type == constants.POWERUP_WILD_BOUNCE:
            return constants.POWERUP_DURATION_WILD_BOUNCE
        elif self.type == constants.POWERUP_CONTROL_SCRAMBLE:
            return constants.POWERUP_DURATION_CONTROL_SCRAMBLE
        return 0
    ...
    def get_color(self):
        ...
        if self.warning_phase:
            ...
            return constants.WHITE if flash else self.get_base_color()
    ...
    def get_base_color(self):
        ...
        # Strategic power-ups (different colors)
            ...
            return constants.NEON_ORANGE  # Orange for swap
        elif self.type == constants.POWERUP_GHOST_BALL:
            return constants.NEON_CYAN    # Cyan for ghost
        elif self.type == constants.POWERUP_MAGNETIZE:
            return constants.NEON_YELLOW  # Yellow for magnetize
        # Chaos power-ups (bright, chaotic colors)
            ...
            return constants.NEON_PURPLE  # Default
