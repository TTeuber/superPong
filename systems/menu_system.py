...
class MenuSystem:
    ...
    def __init__(self):
        # Pause menu state
        self.pause_menu_selected = constants.PAUSE_MENU_RESUME  # Currently selected menu option
    ...
    def handle_pause_menu_input(self, input_handler):
        ...
        button_y_start = constants.SCREEN_HEIGHT // 2 - 60
        button_height = 40
        button_width = 200
        button_x = constants.SCREEN_WIDTH // 2 - button_width // 2
        
        # Check mouse hover and clicks
        for i, option in enumerate(constants.PAUSE_MENU_OPTIONS):
        ...
        if nav_direction != 0:
            if not self.menu_nav_pressed:  # Only trigger on initial press
                self.pause_menu_selected = (self.pause_menu_selected + nav_direction) % len(constants.PAUSE_MENU_OPTIONS)
        ...
        if input_handler.is_menu_cancel_pressed():
            if not self.menu_cancel_pressed:  # Only trigger on initial press
                ...
                self.execute_menu_action(constants.PAUSE_MENU_RESUME)
    ...
    def execute_menu_action(self, action):
        ...
        if action == constants.PAUSE_MENU_RESUME:
            if self.on_resume:
                self.on_resume()
        elif action == constants.PAUSE_MENU_RESTART:
            if self.on_restart:
                self.on_restart()
        elif action == constants.PAUSE_MENU_MAIN_MENU:
            if self.on_main_menu:
                self.on_main_menu()
        elif action == constants.PAUSE_MENU_QUIT:
            if self.on_quit:
                self.on_quit()
                
    def reset_menu(self):
        """Reset menu to default state"""
        self.pause_menu_selected = constants.PAUSE_MENU_RESUME  # Reset menu selection
        self.menu_nav_pressed = False
        self.menu_confirm_pressed = False
        self.menu_cancel_pressed = False